...
"""
Sample data is obtained from the UK Food Standards Agency Food Hygiene Rating Data API, available from: https://ratings.food.gov.uk/open-data/
...
df[['FHRSID','BusinessType','RatingValue','RatingDate','LocalAuthorityName','Hygiene','Structural','ConfidenceInManagement','Longitude','Latitude','Easting','Northing']]\
    .to_csv('sample_data/FoodHygieneRatings_CityOfLondon_accessed%s.csv' % dt.datetime.now().strftime(format='%Y%m%d'), index=False)